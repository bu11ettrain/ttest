...
#	gpu_count=${#busids[@]}
	...
	for(( i=0; i < gpu_count; i++ )); do
		...
		screen -dmS gram_$i bash -c "while true; do node ~/JettonGramGpuMiner/send_universal.js --api tonhub --givers 10000 --allow-shards 1 --bin ~/JettonGramGpuMiner/pow-miner-cuda --gpu $i; sleep 1; done"
	done
 screen -ls
